...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn("inline-flex size-7 items-center justify-center gap-2 rounded-md p-0 text-sm font-medium hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50", className)}
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="sidebar-separator" className={cn("mx-2 w-auto bg-sidebar-border h-px", className)} {...props} />
}
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<"div"> & { showIcon?: boolean }) {
  ...
  return (
    <div
      ...
      {...props}
    >
      {showIcon && <div className="size-4 shrink-0 rounded-md bg-sidebar-accent animate-pulse" />}
      <div className="h-4 max-w-[--skeleton-width] flex-1 rounded-md bg-sidebar-accent animate-pulse" style={{ "--skeleton-width": width } as React.CSSProperties} />
    </div>
  )
}
